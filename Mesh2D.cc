...
void Mesh2D::normalizePts(std::vector<Vertex> &pts)
{
  ...
  {
    pt.position.x -= x_min;
    pt.position.x *= 500.f / (x_max - x_min);
    pt.position.y -= y_min;
    pt.position.y *= 500.f / (y_max - y_min);
  }
}
...
void Mesh2D::buildInnerSquare(Config &cf)
{
       ...
       ++col)
  {
         ...
         ++row)
    {
      ...
      _vertices.push_back(
          new Vertex(sf::Vector2f((1.0 - cf.alpha * sq(abs_x)) * row * cf.inner_block_width,
  ...
  for (size_t col = 0; col != cf.inner_block_count; ++col)
  {
    for (size_t row = 0; row != cf.inner_block_count; ++row)
    {
      ...
      _faces.push_back(new Face{_vertices[base_index],
...
struct CircleCoordinates
{
  ...
  std::pair<double, double> getXY(const long col, const long rad)
  {
    ...
    // const double dr = _outer_radius - _inner_radius;
    // _r =  _inner_radius + _c * (rad * (rad - 1)) * dr /
...
struct SquareAroundCircleCoordinates
{
  ...
  std::pair<double, double> getXY(const long col, const long rad)
  {
    ...
    const double x =
        _circleRadius + rad * (_squareSize - _circleRadius) / static_cast<double>(_numberOfSteps);
    ...
    size_t qdrnt)
{
  for (long col = 0; col != static_cast<long>(cf.inner_block_count) - 1; ++col)
  {
    for (long rad = 0; rad + 2 != static_cast<long>((cf.total_steps)); ++rad)
    {
      ...
      _faces.push_back(new Face{_vertices[base_index_inner],
                                _vertices[base_index_inner + 1],
                                _vertices[base_index_inner + (cf.total_steps)],
  ...
  if (qdrnt < 3)
  {
    if (qdrnt == 0)
    {
      for (long rad = 0; rad + 2 != static_cast<long>((cf.total_steps)); ++rad)
      {
        ...
        _faces.push_back(
            new Face{_vertices[base_index_inner],
                     _vertices[base_index_inner + 1],
                     _vertices[base_index_inner + (cf.inner_block_count - 1) * ((cf.total_steps) - 1) +
                               3 * (cf.inner_block_count) * ((cf.total_steps) - 1) + 1],
                     _vertices[base_index_inner + (cf.inner_block_count - 1) * ((cf.total_steps) - 1) +
                               3 * (cf.inner_block_count) * ((cf.total_steps) - 1)]});
      }
    }
    for (long rad = 0; rad + 2 != static_cast<long>((cf.total_steps)); ++rad)
    {
...
void Mesh2D::buildMesh(Config &cf)
{
  ...
  auto connecting_indices = std::vector<size_t>{cf.inner_block_count,
