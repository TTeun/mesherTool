...
void buildCircleQuadrant(Config &config, size_t quadrant, Mesh2D &mesh)
{
       ...
       ++yIndex)
  {
    ...
    {
      std::tie(x, y) = coordinateHelper.getCoords(radialIndex, CoordinateHelper::R1, quadrant);
    ...
    {
      std::tie(x, y) = coordinateHelper.getCoords(radialIndex, CoordinateHelper::R2, quadrant);
      vertices.push_back(new Vertex(sf::Vector2f(x, y)));
    }

    for (long radialIndex = 0; radialIndex != static_cast<long>(config.outerSteps); ++radialIndex)
    {
      std::tie(x, y) = coordinateHelper.getCoords(radialIndex, CoordinateHelper::R3, quadrant);
      vertices.push_back(new Vertex(sf::Vector2f(x, y)));
    }
  }
}
